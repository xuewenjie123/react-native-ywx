...
import {mysurvey} from '../../services/myDetail';
import NavWait from '../../components/common/NavWait';

var _navigator,_this,_state,_props;

export default class MyInvestigation extends Component {
  ...
  componentDidMount(){
    mysurvey({page:1},_this.getInvestigationList,_this.getInvestigationListFail)
      // InteractionManager.runAfterInteractions(() => {
      // });
  }
  getInvestigationList(result){
    if(result.success&&result.data.records.length){
      if(_state.reset){
        ...
        _this.setState({
          ...
          list_a:_state.list_a.concat(result.data.records),
          total:result.data.total,
          page: result.data.current,
          load: false,
          loading:false,
  ...
  if(_state.page*_state.size<_state.total && !_state.load){
    ...
    mysurvey({page:_this.state.page+1},_this.getInvestigationList,_this.getInvestigationListFail)
  }
  }
     ...
     return(
      <View style={{flex:1}}>
         <View style={styles.repair_loadbox}>
          <Image source={require('../../images/none_investigation.png')} style={styles.load_img} />
          <Text style={styles.load_text}>还没有参加任何调查~</Text>
         </View>
      </View>
     )
   }
