...
export default class WorkOrderApplyModal extends Component {
  constructor(props) {
    super(props);
      ...
      return (
        <TouchableOpacity style={styles.row} onPress={()=>_this.selectWorker(rowID)}>
            <View style={{flexDirection:"row",alignItems:"center",flex:1}}>
                ...
                <Text style={[styles.text1,{marginLeft:30*width/750}]}>{row.typeWork}</Text>
                {row.busyState==0?<Image source={require('../../images/icon_b.png')} style={{width:32*width/750,height:32*width/750,marginLeft:30*width/750}}/>:row.busyState==1?<Image source={require('../../images/icon_b.png')} style={{width:32*width/750,height:32*width/750,marginLeft:30*width/750}}/>:<Image source={require('../../images/icon_b.png')} style={{width:32*width/750,height:32*width/750,marginLeft:30*width/750}}/>}
               <View style={{flexDirection:"row",alignItems:"center",flex:1,justifyContent:"flex-end"}}>
                {row.WillBusyState?<Text style={[styles.text1,{marginRight:20*width/750}]}>{row.WillBusyState}</Text>:null}

                    <Text style={styles.text1}>{row.distance}</Text>
               </View>
            </View>
            {
                rowID==_state.selcetID?<Image source={require('../../images/login_f.png')} style={{width:44*width/750,height:44*width/750,marginLeft:120*width/750}}/>:<View style={{width:width*164/750}}></View>
            }
        </TouchableOpacity>
      )
  }
  render() {
    ...
    return (
        ...
        >
               <TouchableOpacity style={{flex:1, width: width, height: height}} onPress={() => {_props.closeModal()}}>
               </TouchableOpacity>
               <View style={{width:180*width/750,height:180*width/750,alignItems:"center",justifyContent:"center",left:15,top:44+206*width/750,zIndex:10000}}>
