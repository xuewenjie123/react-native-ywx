...
import {getcomplaintList} from '../../services/workList';
...
// ];
export default class ComplaintWorkDetail extends Component {
  ...
  componentDidMount(){
    ...
    InteractionManager.runAfterInteractions(() => {
      getcomplaintList({page:1,status:_state.types},_this.getRepairListSucc,_this.getRepairListFail)
    });
  }
...
getcomplaintList({page:1,status:type},_this.getRepairListSucc,_this.getRepairListFail)
}
onEndReached(){
...
getcomplaintList({page:1,status:_state.types},_this.getRepairListSucc,_this.getRepairListFail)
          ...
          <View style={styles.main}>
              ...
              <View style={styles.list_bar}>
                  <TouchableOpacity style={styles.list_bar_a} onPress={()=>this.changeData('1')}>
                      <Text style={_state.types==1?styles.list_bar_f:styles.list_bar_font}>待处理</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={styles.list_bar_a} onPress={()=>this.changeData('2')}>
                      <Text style={_state.types==2?styles.list_bar_f:styles.list_bar_font}>处理中</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={styles.list_bar_a} onPress={()=>this.changeData('3')}>
                      <Text style={_state.types==3?styles.list_bar_f:styles.list_bar_font}>已处理</Text>
                  </TouchableOpacity>
              </View>
  ...
  _renderRow(list,sectionID,rowID){
    return(
      <TouchableOpacity style={styles.list_a} onPress={()=>_navigator.navigate("MyComplaintDetail",{code:list.code})} key={rowID}>
