...
import {positionList} from '../../services/MyCompany'
 ...
 import { connect } from 'react-redux';
  @connect(({ app }) => ({ ...app }))
export default class ApplySuggest extends Component {
  constructor(props) {
    super(props);
    this.state = {
      complexAddress:"",
  ...
  // applyRepair(path,params,callback,callbackFail){
  ...
  repairAction(){
    var resultFu = function(response){
       if(response.success){
          _navigator.navigate("ApplySuccess",{router:"ApplySuggest"})
      }else{
      ...
      if(!_this.state.complexAddress){
        ...
        console.log(_state.complexAddress,_state.name,_state.tel,_state.sex,_state.message,_state.appointmentTime,_state.audioPath,_state.images,_state.imageData)

        createServer({name:_state.name,sex:_state.sex,phone:_state.tel,workOrderVos:[
          {files:_state.imageData,type:"SUGESSTION",locationId:_state.locationId,
          projectId:_state.projectId,complexAddress:_state.complexAddress,
          audioPath:_state.audioPath}
        ]},resultFu)
        }
